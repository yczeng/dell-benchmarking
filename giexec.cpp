...
class RndInt8Calibrator : public IInt8Calibrator
{
public:
	RndInt8Calibrator(int totalSamples = 16)
	...
	const void* readHistogramCache(size_t&) override
	{
		return nullptr;
	}

	virtual void writeHistogramCache(const void*, size_t) override
	{
	}
...
ICudaEngine* caffeToGIEModel()
{
	...
	{
		Dims3 dims = network->getInput(i)->getDimensions();
	...
	{
		Dims3 dims = network->getOutput(i)->getDimensions();
...
static ICudaEngine* createEngine()
{
	...
	if (!gParams.engine.empty()) {
		...
		engine = infer->deserializeCudaEngine(gieModelStream);
